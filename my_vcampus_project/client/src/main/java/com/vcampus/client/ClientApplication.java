...
public class ClientApplication {
    ...
    private static void setupLookAndFeel() {
        try {
            ...
            log.info("FlatLaf appearance setup completed");
            
        } catch (Exception e) {
            log.warn("Failed to setup FlatLaf, using system default: {}", e.getMessage());
            try {
                // 使用系统默认外观
                UIManager.setLookAndFeel(UIManager.getSystemLookAndFeelClassName());
            } catch (Exception ex) {
                log.error("Failed to setup system look and feel", ex);
            }
        }
    }
    ...
    private static void showLoginInterface() {
        try {
            ...
            log.info("Login interface displayed successfully");
            
        } catch (Exception e) {
            log.error("Failed to show login interface", e);
