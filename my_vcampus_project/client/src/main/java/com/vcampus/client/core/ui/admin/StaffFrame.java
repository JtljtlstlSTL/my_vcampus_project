...
public class StaffFrame extends JFrame {
    private static final Logger logger = LoggerFactory.getLogger(StaffFrame.class);
    ...
    public StaffFrame(NettyClient nettyClient, Map<String, Object> userData) {
        ...
        logger.info("教职工主界面初始化完成: {}", userData.get("userName"));
    }
    ...
    private JPanel createTopPanel() {
        ...
        userInfoPanel.add(lblStaffName);
        userInfoPanel.add(lblStaffId);
    ...
    private JTabbedPane createTabbedPane() {
        ...
        pane.addTab("个人信息", profilePanel);

        // 教学管理面板
        JPanel teachingPanel = createTeachingPanel();
        pane.addTab("教学管理", teachingPanel);

        // 学生管理面板
        JPanel studentMgmtPanel = createStudentManagementPanel();
        pane.addTab("学生管理", studentMgmtPanel);

        // 系统管理面板（如果是管理员角色）
        if (userData.get("roles") != null &&
                ((java.util.List<String>)userData.get("roles")).contains("admin")) {
            JPanel adminPanel = createAdminPanel();
            pane.addTab("系统管理", adminPanel);
        }
    ...
    private JPanel createTeachingPanel() {
        ...
        JLabel titleLabel = new JLabel("教学管理");
        titleLabel.setFont(new Font("微软雅黑", Font.BOLD, 18));
        panel.add(titleLabel, BorderLayout.NORTH);

        // 课程管理部分
        JPanel coursePanel = new JPanel(new BorderLayout());
        coursePanel.setBorder(BorderFactory.createTitledBorder("课程管理"));


        // 课程列表
        String[] columnNames = {"课程ID", "课程名称", "学分", "开课时间", "人数上限", "已选人数"};
        Object[][] data = {
                {"CS101", "计算机导论", "3", "周一 8:00-9:40", "100", "78"},
                {"CS201", "数据结构", "4", "周三 10:00-11:40", "80", "65"},
                {"CS301", "算法设计", "4", "周五 14:00-15:40", "60", "45"}
        };
    ...
    private JPanel createStudentManagementPanel() {
        ...
        // 这里可以添加学生列表和管理功能
        JLabel placeholder = new JLabel("学生管理功能正在开发中...", SwingConstants.CENTER);
    ...
    private JPanel createAdminPanel() {
        ...
        JButton btnUserMgmt = createFeatureButton("用户管理", "users.png");
        JButton btnRoleMgmt = createFeatureButton("角色管理", "roles.png");
        JButton btnSystemLog = createFeatureButton("系统日志", "logs.png");
        JButton btnBackup = createFeatureButton("数据备份", "backup.png");
        JButton btnSettings = createFeatureButton("系统设置", "settings.png");
        JButton btnStats = createFeatureButton("统计报表", "stats.png");

        buttonPanel.add(btnUserMgmt);
        buttonPanel.add(btnRoleMgmt);
        buttonPanel.add(btnSystemLog);
    ...
    private JButton createFeatureButton(String text, String iconName) {
        JButton button = new JButton(text);
        button.setFont(new Font("微软雅黑", Font.PLAIN, 14));

        try {
            ImageIcon icon = new ImageIcon(getClass().getResource("/figures/" + iconName));
            button.setIcon(icon);
    ...
    private Timer statusTimer; // 添加定时器引用
    ...
    private void cleanupResources() {
        try {
            ...
            logger.warn("资源清理过程中发生异常: {}", e.getMessage());
        }
    }
