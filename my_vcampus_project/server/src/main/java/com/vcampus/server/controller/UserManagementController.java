...
public class UserManagementController {
    ...
    public Response addTeacher(Request request) {
        try {
            ...
            // 生成卡号
            String cardNum = UserService.generateCardNum("teacher");
            
            // 创建教师用户
            User newTeacher = new User(cardNum, "123456", name.trim(), 25,
    ...
    public Response addStudent(Request request) {
        try {
            ...
            // 生成卡号
            String cardNum = UserService.generateCardNum("student");
            
            // 创建学生用户
            User newStudent = new User(cardNum, "123456", name.trim(), 20,
                                      Gender.valueOf(gender), phone, 
    ...
    public Response deleteUser(Request request) {
        try {
            ...
            boolean success = UserService.deleteUser(cardNumStr);
            
            if (success) {
                log.info("管理员删除用户成功：{}", cardNumStr);
